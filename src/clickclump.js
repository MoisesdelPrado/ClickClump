...
chrome.runtime.sendMessage({
	...
	if (response === null) {
		console.log("Unable to load clickclump due to null response");
	} else {
		...
		try {
			...
			for (var i in response.blocked) {
				...
				if (re.test(window.location.href)) {
					allowed = false;
					console.log("Clickclump is blocked on this site: " + response.blocked[i] + "~" + window.location.href);
				}
			}
			...
			console.log("Unable to properly load clickclump, returning to default settings: " + JSON.stringify(response));
		}
	}
});
