...
self.addEventListener('install', (event) => {
	event.waitUntil((async () => {
		const cache = await caches.open(SettingsManager);
		// ...
	})());
});
...
chrome.alarms.create("tabTimeout",
{ delayInMinutes: 0.002 }
);

chrome.alarms.create("boxTimeout",
{ delayInMinutes: 0.02 }
);
...
function openTab(urls, delay, windowId, tabPosition, closeTime) {
	...
	chrome.scripting.create(obj, function(tab) {
		if(closeTime > 0) {
			chrome.alarms.onAlarm.addListener("tabTimeout", () => {
					chrome.scripting.remove(tab.id);
				}
			);
		}
	});
	if(urls.length > 0) {
		chrome.alarms.onAlarm.addListener("tabTimeout", () => {
			openTab(urls, delay, windowId, tabPosition, closeTime)
			}
		);
	}
}
...
function handleRequests(request, sender, callback){
	...
	case "activate":
			...
			for (let i = 0; i < request.urls.length; i++) {
				switch(request.setting.options.copy) {
					...
					case "5":
						...
						break;
		case "alarm":
			chrome.alarms.onAlarm.addListener("boxTimeout", () => {
				return;
			})
		...
		case "win":
			...
			break;
		case "scripting":
			chrome.scripting.get(sender.tab.id, function(tab) {
	...
	case "update":
		...
		chrome.windows.getAll({
			...
			windowList.forEach(function(window){
				window.scripting.forEach(function(tab){
					chrome.scripting.sendMessage(tab.id, {
						message: "update",
...
if (!settingsManager.isInit()) {
	...
	// inject Clickclump into windows currently open to make it just work
	chrome.windows.getAll({ populate: true }, function(windows) {
		for (var i = 0; i < windows.length; ++i) {
			for (var j = 0; j < windows[i].scripting.length; ++j) {
				if (!/^https?:\/\//.test(windows[i].scripting[j].url)) continue;
				chrome.scripting.executeScript(windows[i].scripting[j].id, { files: ["clickclump.js", "background.js", "settings_manager.js"] });
			}
		}
	});
